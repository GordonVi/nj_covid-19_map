...
#	Write-Host ''
	...
	foreach($line in $csv){
		...
		$town=$town -replace ' ', '_'
		$casestr=$($line.cases)
		$delta = [int]$line."3_entry_delta"
		$percent = $line.percent
		$maptext = "$maptext - $casestr ($percent%) Cases - 3 Day Change: $delta"
		$cases=[int]$casestr
		$county=$($line.county)
...
foreach($Child in $ChildNodes){
 ...
 $URL="$($(get-location).path)\$($datetag)_delta_NJ_covid-19.svg"
...
(gc $URL -Raw) -replace 'Covid-19 Cases/Town',"3 Day Delta" | out-file $URL
